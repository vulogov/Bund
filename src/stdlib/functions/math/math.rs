            ...
            match value.cast_float() {
                Ok(fvalue) => {
                    match op {
                        ...
                        Ops::Fact => {
                            vm.stack.push(Value::from_float(math::fact(fvalue as u64) as f64));
                        }
                        Ops::Ln => {
                        ...
                        Ops::Perimeter => {
                            match vm.stack.pull() {
                                Some(value2) => {
                                    match value2.cast_float() {
                                        Ok(yvalue) => {
                                            vm.stack.push(Value::from_float(math::perimeter(fvalue, yvalue)));
                                        }
                                        Err(err) => {
                                            bail!("FLOAT_OP returns error: {}", err);
                                        }
...
pub fn init_stdlib(cli: &cmd::Cli) {
    ...
    let _ = bc.vm.register_inline("math.exp".to_string(), stdlib_float_exp_inline);
    let _ = bc.vm.register_inline("math.factorial".to_string(), stdlib_float_fact_inline);
    let _ = bc.vm.register_inline("math.ln".to_string(), stdlib_float_ln_inline);
    let _ = bc.vm.register_inline("math.log10".to_string(), stdlib_float_log10_inline);
    let _ = bc.vm.register_inline("math.nroot".to_string(), stdlib_float_nroot_inline);
    let _ = bc.vm.register_inline("math.perimeter".to_string(), stdlib_float_perimeter_inline);
    let _ = bc.vm.register_inline("math.power".to_string(), stdlib_float_pow_inline);
}
