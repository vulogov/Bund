...
pub fn bund_save_bootstrap_inline<'a>(vm: &'a mut VM, conn: &mut Connection, name: String, snippet: String) -> Result<&'a mut VM, Error> {
    match helpers::world::bootstrap::save_bootstrap(vm, conn, name, snippet) {
        Ok(_) => {}
        Err(err) => {
            bail!("Bootstrap SAVE.SCRIPT returns: {}", err)
        }
    }
    Ok(vm)
}

pub fn bund_load_bootstrap_inline<'a>(vm: &'a mut VM, conn: &mut Connection, name: String) -> Result<&'a mut VM, Error> {
    match helpers::world::bootstrap::load_bootstrap(vm, conn, name) {
        Ok(_) => {}
        Err(err) => {
            bail!("Bootstrap LOAD.SCRIPT returns: {}", err)
        }
    }
    Ok(vm)
}
...
pub fn stdlib_bund_load_bootstrap(vm: &mut VM) -> Result<&mut VM, Error> {
    if vm.stack.current_stack_len() < 2 {
        bail!("Stack is too shallow for LOAD.SCRIPT");
    }
    let file_name_value = match vm.stack.pull() {
        Some(file_name_value) => file_name_value,
        None => {
            bail!("LOAD.SCRIPT returns NO DATA #1");
        }
    };
    let name_value = match vm.stack.pull() {
        Some(name_value) => name_value,
        None => {
            bail!("LOAD.SCRIPT returns NO DATA #3");
        }
    };
    ...
    let name = match name_value.cast_string() {
        Ok(name) => name,
        Err(err) => {
            bail!("LOAD.SCRIPT casting string #3 returns: {}", err);
        }
    };
    let file_name = match file_name_value.cast_string() {
        Ok(file_name) => file_name,
        Err(err) => {
            bail!("LOAD.SCRIPT casting string #1 returns: {}", err);
        }
    };
    let mut conn = match helpers::world::open(file_name) {
        Ok(conn) => conn,
    ...
    let res = bund_load_bootstrap_inline(vm, &mut conn, name);
    match conn.close() {
        Ok(_) => {},
        Err(err) => {
            log::debug!("Closing connection to the world returns error: {:?}", err);
...
pub fn stdlib_bund_save_script_disabled(_vm: &mut VM) -> Result<&mut VM, Error> {
    bail!("bund [SAVE|LOAD].SCRIPT functions disabled with --noio");
}
...
pub fn init_stdlib(cli: &cmd::Cli) {
    ...
    if cli.noio {
        let _ = bc.vm.register_inline("save.script".to_string(), stdlib_bund_save_script_disabled);
        let _ = bc.vm.register_inline("load.script".to_string(), stdlib_bund_save_script_disabled);
    } else {
        let _ = bc.vm.register_inline("save.script".to_string(), stdlib_bund_save_bootstrap);
        let _ = bc.vm.register_inline("load.script".to_string(), stdlib_bund_load_bootstrap);
    }
    drop(bc);
}
