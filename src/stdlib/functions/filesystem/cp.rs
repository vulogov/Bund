...
pub fn stdlib_bund_file_cp_base(vm: &mut VM, op: FsOperations, err_prefix: String) -> Result<&mut VM, Error> {
    ...
    match file_name_value.type_of() {
        ...
        LIST => {
            match file_name_value.cast_list() {
                Ok(file_name_list) => {
    ...
    log::debug!("{} src: {:?} target: {}", &err_prefix, &from_paths, &target_file_name);
    let res = match op {
...
pub fn init_stdlib(cli: &cmd::Cli) {
    ...
    if cli.noio {
        let _ = bc.vm.register_inline("fs.cp".to_string(), stdlib_bund_file_cp_disabled);
        let _ = bc.vm.register_inline("fs.mv".to_string(), stdlib_bund_file_cp_disabled);
        let _ = bc.vm.register_inline("fs.rm".to_string(), stdlib_bund_file_cp_disabled);
    } else {
        let _ = bc.vm.register_inline("fs.cp".to_string(), stdlib_bund_file_cp_inline);
        let _ = bc.vm.register_inline("fs.mv".to_string(), stdlib_bund_file_mv_inline);
        let _ = bc.vm.register_inline("fs.rm".to_string(), stdlib_bund_file_rm_inline);
    }
    drop(bc);
}
