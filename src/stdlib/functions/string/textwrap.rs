...
pub fn string_wrap_base(vm: &mut VM, op: StackOps, lang: Language, err_prefix: String) -> Result<&mut VM, Error> {
    match op {
        StackOps::FromStack => {
    ...
    let options = Options::new(n as usize)
...
pub fn stdlib_string_stack_wrap_english_inline(vm: &mut VM) -> Result<&mut VM, Error> {
    string_wrap_base(vm, StackOps::FromStack, Language::EnglishUS, "STRING.WRAP.ENGLISH".to_string())
}

#[time_graph::instrument]
pub fn stdlib_string_workbench_wrap_english_inline(vm: &mut VM) -> Result<&mut VM, Error> {
    string_wrap_base(vm, StackOps::FromWorkBench, Language::EnglishUS, "STRING.WRAP.ENGLISH.".to_string())
}
...
pub fn init_stdlib(cli: &cmd::Cli) {
    ...
    let _ = bc.vm.register_inline("string.wrap.english".to_string(), stdlib_string_stack_wrap_english_inline);
    let _ = bc.vm.register_inline("string.wrap.english.".to_string(), stdlib_string_workbench_wrap_english_inline);
