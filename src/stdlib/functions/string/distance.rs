...
pub fn string_distance_base(vm: &mut VM, op: StackOps, aop: DistanceAlgorithm, err_prefix: String) -> Result<&mut VM, Error> {
    ...
    let res: Value = match aop {
        DistanceAlgorithm::Levenshtein => Value::from_int(distance::levenshtein(&string1_data, &string2_data) as i64),
        DistanceAlgorithm::DamerauLevenshtein => Value::from_int(distance::damerau_levenshtein(&string1_data, &string2_data) as i64),
        DistanceAlgorithm::Hamming => {
            Value::from_int(
                match distance::hamming(&string1_data, &string2_data) {
                    Ok(dist) => dist,
                    Err(err) => {
...
pub fn stdlib_string_stack_distance_lev_inline(vm: &mut VM) -> Result<&mut VM, Error> {
    string_distance_base(vm, StackOps::FromStack, DistanceAlgorithm::Levenshtein, "STRING.DISTANCE.LEVENSHTEIN".to_string())
}
pub fn stdlib_string_wb_distance_lev_inline(vm: &mut VM) -> Result<&mut VM, Error> {
    string_distance_base(vm, StackOps::FromWorkBench, DistanceAlgorithm::Levenshtein, "STRING.DISTANCE.LEVENSHTEIN.".to_string())
}

pub fn stdlib_string_stack_distance_dlev_inline(vm: &mut VM) -> Result<&mut VM, Error> {
    string_distance_base(vm, StackOps::FromStack, DistanceAlgorithm::DamerauLevenshtein, "STRING.DISTANCE.DAMERAULEVENSHTEIN".to_string())
}
pub fn stdlib_string_wb_distance_dlev_inline(vm: &mut VM) -> Result<&mut VM, Error> {
    string_distance_base(vm, StackOps::FromWorkBench, DistanceAlgorithm::DamerauLevenshtein, "STRING.DISTANCE.DAMERAULEVENSHTEIN.".to_string())
}

pub fn stdlib_string_stack_distance_ham_inline(vm: &mut VM) -> Result<&mut VM, Error> {
    string_distance_base(vm, StackOps::FromStack, DistanceAlgorithm::Hamming, "STRING.DISTANCE.HAMMING".to_string())
}
pub fn stdlib_string_wb_distance_ham_inline(vm: &mut VM) -> Result<&mut VM, Error> {
    string_distance_base(vm, StackOps::FromWorkBench, DistanceAlgorithm::Hamming, "STRING.DISTANCE.HAMMING.".to_string())
}

pub fn stdlib_string_stack_distance_sift_inline(vm: &mut VM) -> Result<&mut VM, Error> {
    string_distance_base(vm, StackOps::FromStack, DistanceAlgorithm::Sift3, "STRING.DISTANCE.SIFT3".to_string())
}
...
pub fn init_stdlib(cli: &cmd::Cli) {
    ...
    let _ = bc.vm.register_inline("string.distance.levenshtein".to_string(), stdlib_string_stack_distance_lev_inline);
    let _ = bc.vm.register_inline("string.distance.levenshtein.".to_string(), stdlib_string_wb_distance_lev_inline);
    let _ = bc.vm.register_inline("string.distance.dameraulevenshtein".to_string(), stdlib_string_stack_distance_dlev_inline);
    let _ = bc.vm.register_inline("string.distance.dameraulevenshtein.".to_string(), stdlib_string_wb_distance_dlev_inline);
    let _ = bc.vm.register_inline("string.distance.hamming".to_string(), stdlib_string_stack_distance_ham_inline);
