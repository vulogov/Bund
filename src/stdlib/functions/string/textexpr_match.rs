...
pub fn string_srch_match_base(vm: &mut VM, op: StackOps, err_prefix: String) -> Result<&mut VM, Error> {
    match op {
        StackOps::FromStack => {
    ...
    let matcher = match srch::Expression::new(&string1_data) {
        Ok(matcher) => matcher,
        Err(err) => bail!("{} returned error when creates matcher: {:?}", &err_prefix, err),
    };
    let _ = match op {
        StackOps::FromStack => vm.stack.push(Value::from_bool(matcher.matches(string2_data))),
        StackOps::FromWorkBench => vm.stack.push_to_workbench(Value::from_bool(matcher.matches(string2_data))),
    };
    Ok(vm)
}
...
pub fn stdlib_string_stack_srch_inline(vm: &mut VM) -> Result<&mut VM, Error> {
    string_srch_match_base(vm, StackOps::FromStack, "STRING.EXPRESSIONMATCH".to_string())
}

#[time_graph::instrument]
pub fn stdlib_string_workbench_srch_inline(vm: &mut VM) -> Result<&mut VM, Error> {
    string_srch_match_base(vm, StackOps::FromWorkBench, "STRING.EXPRESSIONMATCH.".to_string())
}
...
pub fn init_stdlib(cli: &cmd::Cli) {
    ...
    let _ = bc.vm.register_inline("string.expressionmatch".to_string(), stdlib_string_stack_srch_inline);
    let _ = bc.vm.register_inline("string.expressionmatch.".to_string(), stdlib_string_workbench_srch_inline);
