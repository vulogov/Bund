...
pub fn string_system_path_base(vm: &mut VM, op: StackOps, pop: UnixPathAlgorithm, err_prefix: String) -> Result<&mut VM, Error> {
    match op {
        StackOps::FromStack => {
...
pub fn stdlib_system_path_split_stack(vm: &mut VM) -> Result<&mut VM, Error> {
    string_system_path_base(vm, StackOps::FromStack, UnixPathAlgorithm::Split, "SYSTEM.PATH.SPLIT".to_string())
}
pub fn stdlib_system_path_split_workbench(vm: &mut VM) -> Result<&mut VM, Error> {
    string_system_path_base(vm, StackOps::FromWorkBench, UnixPathAlgorithm::Split, "SYSTEM.PATH.SPLIT.".to_string())
}

pub fn stdlib_system_path_filename_stack(vm: &mut VM) -> Result<&mut VM, Error> {
    string_system_path_base(vm, StackOps::FromStack, UnixPathAlgorithm::Filename, "SYSTEM.PATH.FILENAME".to_string())
}
pub fn stdlib_system_path_filename_workbench(vm: &mut VM) -> Result<&mut VM, Error> {
    string_system_path_base(vm, StackOps::FromWorkBench, UnixPathAlgorithm::Filename, "SYSTEM.PATH.FILENAME.".to_string())
}
...
pub fn init_stdlib(cli: &cmd::Cli) {
    ...
    let _ = bc.vm.register_inline("system.path.split".to_string(), stdlib_system_path_split_stack);
    let _ = bc.vm.register_inline("system.path.split.".to_string(), stdlib_system_path_split_workbench);
    let _ = bc.vm.register_inline("system.path.filename".to_string(), stdlib_system_path_filename_stack);
    let _ = bc.vm.register_inline("system.path.filename.".to_string(), stdlib_system_path_filename_workbench);
    drop(bc);
}
